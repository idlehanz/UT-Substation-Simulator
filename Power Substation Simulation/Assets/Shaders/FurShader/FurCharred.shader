Shader "Example/CharredFur" {
	Properties{
		_MainTex("FurColor", 2D) = "white" {}
		_FurHeight("Fur Height",2D) = "white"{}
		_CharMap("Char Map",2D) = "white"{}
		_Amount("Height", Range(0,1)) = 0.5
		_CharAmount("Charred Amount", Range(0,1)) = 0.5
	}
		SubShader{
...
#pragma surface surf Lambert vertex:vert
		...
		void surf(Input IN, inout SurfaceOutput o) {
			...
			float4 charColor = tex2D(_FurHeight, IN.uv_MainTex);
			...
			//	discard;
			o.Albedo = mainColor - (charColor*_CharAmount);
			o.Alpha = 1.1 - height;
		}
		ENDCG
...
#pragma Standard alpha
	...
	void surf(Input IN, inout SurfaceOutput o) {
		...
		fixed4 cc = tex2D(_CharMap, IN.uv_MainTex);
		//clip(frac((IN.worldPos.y + IN.worldPos.z*0.1) * 5) - 0.5);
		o.Albedo = c.rgb -(cc.rgb*_CharAmount);
	}
	ENDCG
	}
		Fallback "Diffuse"
